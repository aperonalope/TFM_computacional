...
for files in csvs:
    ...
    matches=df.groupby(by=df.columns[0],as_index=True).sum('nºMatches')
    n_matches=matches['nºMatches'].value_counts().sort_index().to_numpy()
    #print(n_matches)
    ...
    plt.axis('equal')
    plt.axis('equal')
    plt.legend(labels, bbox_to_anchor=(0.84, 0.7),title="nº matches",fontsize=18,title_fontsize=24)
    plt.title(files.split('_')[1]+' '+files.split('_')[2],fontsize=28)

    plt.savefig('pie_chart'+'_'+files.split('_')[1]+'_'+files.split('_')[2]+'.png',dpi=1000)
    plt.clf()
